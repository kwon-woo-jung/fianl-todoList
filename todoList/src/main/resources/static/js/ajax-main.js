...
const popupTodoComplete = document.querySelector("#popupTodoComplete");
const popupRegDate = document.querySelector("#popupRegDate");
const popupTodoDetail = document.querySelector("#popupTodoDetail");
const popupClose = document.querySelector("#popupClose");

// 상세 조회 팝업레이어 관련 버튼 요소
const changetodoComplete = document.querySelector("#changetodoComplete");
...
// completeCount 값 비동기 통신으로 얻어와 화면 추렭
...
addBtn.addEventListener("click", () => {
  ...
  if(todoTitle.value.trim().length === 0 || todoDetail.value.trim().length === 0) {
  ...
  // - 전달 데이터 : todoTitle.value, todoDetail.value
  // JSON
  // { "name" : "홍길동",
  //  "age" : 20,  
  ...
  //  todoTitle과 todoDetail를 저장한 JS 객체
  const param = {
    // key    :   value
...
const selectTodo = (url) => {
  ...
  .then(todo => {
    ...
    popupTodoComplete.innerText   = todo.todoComplete;
...
// 완료 여부 변경 버튼 클릭 시
changetodoComplete.addEventListener("click", () => {

  // 변경할 할 일 번호, 완료 여부  (Y <-> N)
  const todoNo = popupTodoNo.innerText;
  const todoComplete = popupTodoComplete.innerText === 'Y' ? 'N' : 'Y';

  // SQL 수행에 필요한 두 값을 JS 객체로 묶음
  const obj = {"todoNo" : todoNo, "todoComplete" : todoComplete};

  // 비동기로 완료 여부 변경 요청
  fetch("/ajax/changeTodoComplete", {
    method: "PUT", // @PutMapping
    headers : {"TodoContent-Type" : "application/json"},
  ...
  .then(  result => {
    ...
    if(result > 0) { // 성공
      ...
      popupTodoComplete.innerText = todoComplete;
      ...
      if(Comment === 'Y') completeCount.innerText = count + 1;
...
// 수정 레이어에서 취소버튼(#updateCancel) 이 클릭되었을떄
updatrCancel.addEventListener("click", () => {
...
updateBtn.addEventListener("click", e => {
  ...
  .then( result => {
    ...
    if(result > 0) {
      ...
      updateBtn.removeAttribute("data-todo-no"); // 속성제거거
